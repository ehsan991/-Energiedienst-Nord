...
export default async function Home({ params }: { params: Promise<{ locale: 'de'|'en' }> }) {
  ...
  return <>
    ...
    <section className="trust-section"><div className="shell trust-grid"><div className="trust-heading"><p className="eyebrow">{t.home.trustEyebrow}</p><h2>{t.home.trustTitle}</h2></div><div className="trust-copy"><p>{t.home.trustText1}</p><p>{t.home.trustText2}</p><strong>{t.home.trustClaim}</strong></div></div><div className="shell trust-principles">{t.home.trustPrinciples.map((item:any)=><div key={item}><Check size={16}/><span>{item}</span></div>)}</div></section>
